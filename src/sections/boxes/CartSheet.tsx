import { ShoppingCart, Trash2, Info, CreditCard, ArrowRight, Truck } from 'lucide-react';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
import { RENTAL_TERM_DAYS } from '@/data/rentals';
      ...
      <SheetContent side="right" className="w-full sm:max-w-md bg-[#F3F3F1] p-0 gap-0 flex flex-col">
        <SheetHeader className="px-6 pt-6 pb-4 border-b border-gray-200 gap-1">
          ...
          <SheetDescription className="text-sm text-gray-500">
            Mix bundles, individual boxes, supplies and bin rentals in one order.
          </SheetDescription>
        </SheetHeader>
        ...
        {lines.length > 0 && (
          <SheetFooter className="border-t border-gray-200 bg-white px-6 py-5 gap-4">
            {/* Rental lines carry a real price, so they don't fork the cart —
                they only add the terms that come with renting instead of
                buying. Rental fulfilment (term, delivery window, pickup,
                equipment count) is phase-2 work; until then this cart ends on
                the same "coming soon" state as a cardboard-only order. */}
            {hasRental && (
              <div
                role="status"
              ...
              >
                ...
                <div>
                  <p className="text-sm font-bold text-[#0A0A0A] mb-1">
                    Your order includes a {RENTAL_TERM_DAYS}-day bin rental
                  </p>
                  <p className="text-xs text-gray-600 leading-relaxed">
                    Delivery, pickup and the dollies listed on your package are included within our
                    standard service area. Your delivery window will be confirmed before delivery.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="w-full space-y-2.5">
              {/*
                ...
                TODO: CHECKOUT DE ALUGUEL (fase futura) — rental lines cannot
                simply be charged like a box: the order also carries a rental
                term, a delivery window, a pickup and an equipment count, and
                extensions are billed per additional week. Do NOT quietly let
                rental lines through a cardboard-only Stripe flow when it
                lands; rental fulfilment gets its own pass.
              */}
              <button
                type="button"
                disabled
              ...
              <p className="text-[10px] text-gray-500 text-center leading-relaxed">
                {hasRental
                  ? 'Online payment for boxes, supplies and bin rentals is coming soon. Call or text us to book your rental in the meantime.'
                  : 'Secure online payment for boxes and supplies is coming soon. Your cart is saved while you keep browsing.'}
              </p>
            </div>
          </SheetFooter>
        )}
